...
contract BaseCore is Ownable, Pausable, ReentrancyGuard {
    ...
    event OneSwapped(address indexed srcToken, address indexed dstToken, address indexed dstReceiver, uint256 amount, uint256 returnAmount, uint256 toChainID, string channel);

    constructor() Ownable(msg.sender) {
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                keccak256("EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)"),
                keccak256(bytes("1SwapV5")),
                keccak256(bytes("5")),
                block.chainid,
                address(this)
            )
    ...
    function _emitOneSwap(address srcToken, address dstToken, address dstReceiver, uint256 amount, uint256 returnAmount, uint256 toChainID, string memory channel) internal {
        emit OneSwapped(
            srcToken,
            dstToken,
            dstReceiver,
    ...
    function changeOneSwapProxy(address aggregator, address signer, address vault) external onlyExecutor {
    ...
    function oneSwapProxyAddress() external view returns (address bridgeProxy, address feeSigner) {
        bridgeProxy = _aggregate_bridge;
        feeSigner = _fee_signer;
    }

    function oneSwapFee() external view returns (uint256, uint256, address) {
        return (_aggregate_fee, _cross_fee, _vault);
    }

    function oneSwapAllowedQuery(address crossCaller, address wrappedToken, uint256 poolIndex) external view returns (bool isCrossCallerAllowed, bool isWrappedAllowed, UniswapV3Pool memory pool) {
